...
func _get_meta_progress() -> MetaProgress:
    ...
    if meta == null:
        print("TotemSelect: MetaProgress not available, using fallback totem list")
    return meta

func _add_default_heartseed_entry(meta: MetaProgress = null) -> void:
    if meta != null:
        for def in meta.all_totem_defs:
            if str(def.get("id", "")) == "totem.heartseed":
                ...
                if not fallback_entry.has("name"):
                    fallback_entry["name"] = "Heartseed Totem"
                _totem_entries.append(fallback_entry)
                print("TotemSelect: added fallback unlocked Heartseed totem from MetaProgress")
                return
    var default_entry := {
        "id": "totem.heartseed",
    ...
    _totem_entries.append(default_entry)
    print("TotemSelect: added default fallback Heartseed totem")
...
func _load_totems_from_meta() -> void:
    ...
    if meta != null:
        ...
        print("TotemSelect: ERROR - MetaProgress singleton not found, using empty list")
        _totem_entries = []

    var any_unlocked := false
    for e in _totem_entries:
        if e is Dictionary and bool(e.get("unlocked", false)):
            any_unlocked = true
            break
    ...
    if not any_unlocked:
        for i in range(_totem_entries.size()):
            ...
            if e is Dictionary and str(e.get("id", "")) == "totem.heartseed":
                ...
                any_unlocked = true
                print("TotemSelect: fallback - forcing totem.heartseed unlocked for selection")
                break

    if not any_unlocked:
        _add_default_heartseed_entry(meta)
